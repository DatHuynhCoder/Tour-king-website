...
const PaymentPage = () => {
  ...
  useEffect(() => {
    ...
    if (MaNguoiDung) {
      axios.get(`http://localhost:8800/get-user-by-id?userid=${MaNguoiDung}`).then(res => {
        ...
        const [year, month, day] = res.data[0].NgaySinh.split('-');
        setDay(day);
        setMonth(month);
        setYear(year);
        // setFullName(res.data[0].TenDayDu);
        // setHC(res.data[0].MaHoChieu);
        // setQT(res.data[0].QuocTich);
        // setPhone(res.data[0].SDT);
      })
      //Sang xu ly luon ngay sinh user
    }
      ...
      axios.post('http://localhost:8800/add-ctdv', {
        ...
        if (res.data.Status === 'Error') {
          ...
          switch (paymentMethod) {
            case 'momo':
              toast.success('Payment with MoMo successful!');
              break;
            case 'vnpay':
              toast.success('Payment with VNPay successful!');
              break;
            case 'counter':
              toast.success('Payment at the counter successful!');
              break;
            default:
  ...
  const handleChangeUserInfo = (info) => {
    ...
    // setFullName(info.TenDayDu);
    // setHC(info.MaHoChieu);
    // setQT(info.QuocTich);
    // setPhone(info.SDT);
    const [yearCustomer, monthCustomer, dayCustomer] = info.NgaySinh.split('-');
    setDay(dayCustomer);
    setMonth(monthCustomer);
    setYear(yearCustomer);
  }
  return (
    <>
      <Modal
      ...
      <div className="invoice-container">
        ...
        <div className="left-column">
          ...
          <div className="customer-info">
            ...
            >Chọn nhanh thông tin khách hàng</Button>
          </div>
